...
ABT.init = function(){
	if(ABT.isIEBelow10()){
		console.log('当前浏览器版本低于IE10，无障碍功能加载失败！');
		return false;
	}
	//工具条界面状态--------------------------
...
//###########################[检测浏览器版本]###########################
ABT.isIEBelow10 = function() {
    var userAgent = navigator.userAgent;  
    var msie = userAgent.indexOf('MSIE ');  
    var trident = userAgent.indexOf('Trident/');  
  
    if (msie > 0) {  
        var rv = userAgent.substring(msie + 5, userAgent.indexOf('.', msie));  
        return parseInt(rv, 10) < 10;  
    }
    if (trident > 0) {   
        var rv = userAgent.indexOf('rv:');  
        if (rv > 0) {  
            var version = userAgent.substring(rv + 3, userAgent.indexOf('.', rv));  
